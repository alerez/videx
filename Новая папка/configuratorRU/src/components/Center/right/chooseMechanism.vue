    ...
    <div class="selectFrameBlock">
      <div style="display:flex">
        <div class="selectChooseFrame" @click="getMechanismColor('white')">
          ...
          <p v-if="mechanismColor !== 'white'" class="selectFrameText">{{ chooseFrame.plastic.white.description.ru }}</p>
          <p v-if="mechanismColor === 'white'" style="color:#FF7A00" class="selectFrameText">{{ chooseFrame.plastic.white.description.ru }}</p>
        </div>
        <div class="selectChooseFrame" @click="getMechanismColor('silver')">
          ...
          <p v-if="mechanismColor !== 'silver'" class="selectFrameText">{{ chooseFrame.plastic.silver.description.ru }}</p>
          <p v-if="mechanismColor === 'silver'" style="color:#FF7A00" class="selectFrameText">{{ chooseFrame.plastic.silver.description.ru }}</p>
        </div>
      </div>
      <div style="display:flex">
        <div class="selectChooseFrame" @click="getMechanismColor('crema')">
          ...
          <p v-if="mechanismColor !== 'crema'" class="selectFrameText">{{ chooseFrame.plastic.crema.description.ru }}</p>
          <p v-if="mechanismColor === 'crema'" style="color:#FF7A00" class="selectFrameText">{{ chooseFrame.plastic.crema.description.ru }}</p>
        </div>
        <div class="selectChooseFrame" @click="getMechanismColor('black')">
          ...
          <p v-if="mechanismColor !== 'black'" class="selectFrameText">{{ chooseFrame.plastic.black.description.ru }}</p>
          <p v-if="mechanismColor === 'black'" style="color:#FF7A00" class="selectFrameText">{{ chooseFrame.plastic.black.description.ru }}</p>
        </div>
      </div>
    </div>
    <div class="chooseMechanismBlockTwoRam">
      <div class="chooseMechanismBlockTwo chooseMechanismBlockMobile">
        <div class="chooseMechanismFlex" v-for="(index, idx) in chooseMechanism.plastic[mechanismColor]" v-bind:key="idx" v-on:click="doneChooseMechanism(index)">
          ...
          <p class="chooseMechanismFlexP">{{index.description.ru}}</p>
        </div>
      </div>
    </div>
  </div>
...
export default {
  data(){
    return{
      chooseMechanismText: 'Виберите механизм',
    }
  },
  computed: {
  ...
  methods: {
    ...mapActions(['EMIT_selectedMechanism', 'EMIT_mechanism', 'SET_MECHANISMS', 'EMIT_mechanismColor']),
    doneChooseMechanism: function doneChooseMechanism(data){
    ...
    getMechanismColor: function getMechanismColor(data){
      const data2 = data
      this.EMIT_mechanismColor(data, data2)
    }
  }
};
</script>
...
.chooseMechanismBlockTwo::-webkit-scrollbar {
  width: 16px;
}
.chooseMechanismBlockTwo::-webkit-scrollbar-track {
...
.chooseMechanismBlockTwo::-webkit-scrollbar-thumb {
...
.chooseMechanismColorBlock{
  display: flex;
  flex-basis: 50%;
  margin:10px;
}
.chooseMechanismBlockColorBlock{
  border-radius: 50%;
  border: 1px solid rgb(167,169,171);
  width: 40px;
  height: 40px;
}
.chooseMechanismColorText{
  font-family: Inter, sans-serif;
  font-style: normal;
  font-weight: normal;
  font-size: 16px;
...
.chooseMechanismBlockColorWithe{
  background-color: rgb(255,255,255);
}
.chooseMechanismBlockColorCreme{
  background-color: rgb(242,224,211);
}
.chooseMechanismBlockColorBlack{
  background-color: rgb(35,31,32);
}
.chooseMechanismBlockColorGray{
